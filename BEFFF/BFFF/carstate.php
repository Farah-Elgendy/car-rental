...
$rows_count_value = mysqli_num_rows($mysqliStatus);
 return $rows_count_value;
  ...
  <?php 
	...
	$query="SELECT * FROM car JOIN reservation USING(plate_number)
  WHERE 
 ( `start_date` > '$start_date') 
   ORDER BY `start_date`";
    ...
    if ($result->num_rows > 0) {
        ...
        . "</td><td>". $row["seats"]."</td><td>".$row["state"] . "</td><td>". $row["insurance"]."</td><td>". $row["start_date"] 
...
if ($result=mysqli_query($connection,$sql)) {
    ...
    echo "Number of Reservations ".$rowcount."       "; 
}
?></div>
                                </div>
                                <div class="col-auto">
